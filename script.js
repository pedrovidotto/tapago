document.addEventListener('DOMContentLoaded', () => {
    ...
    const frasesMotivacionais = [
        "A jornada começa com um passo.", "Consistência é a chave.", "Acredite no seu potencial.", "Cada gota de suor conta.", "Feito é melhor que perfeito.", "Dor hoje, força amanhã.", "Mente desiste antes do corpo.", "Hábito > Motivação.", "Pequenos progressos = Grandes resultados.", "Corpo alcança o que a mente acredita.", "Aumente o esforço.", "Crie sua força de vontade.", "Faça o que nunca fez.", "Constância no propósito."
    ]; // Frases mais curtas
    const mensagensDeConclusao = [
        "Mandou bem! Descanse e volte com tudo!", "Treino concluído! Vitória do dia. Até a próxima!", "Parabéns! Consistência é tudo. Volte amanhã!", "Excelente! O esforço de hoje vale a pena. Nos vemos!", "Missão cumprida! Orgulhe-se. Te esperamos!"
    ];

    const dadosTreino = [ // Adiciona nomeCompleto para os botões inferiores
    ...
    const elementos = {
        ...
        workoutCard: document.getElementById('workout-card'), // Referência ao card principal
    ...
    const carregarProgresso = () => { progresso = JSON.parse(localStorage.getItem('minhaRotinaCleanV3')) || {}; };
    const salvarProgresso = () => { localStorage.setItem('minhaRotinaCleanV3', JSON.stringify(progresso)); };
    ...
    const atualizarVisualCardExercicio = (card, id, ex) => {
        ...
        if (fillElement) fillElement.style.width = `${porcentagem}%`; // Usa width para fundo
        if (counterElement) counterElement.textContent = `${seriesFeitas}/${ex.series}`; // Atualiza texto
    ...
    const renderizarTreino = (index) => {
        ...
        if (elementos.completedList) elementos.completedList.innerHTML = ''; // Limpa concluídos
        ...
        diaData.exercicios.forEach((ex, exIndex) => {
            ...
            li.innerHTML = `
                ...
                <div class="detalhes-exercicio">
                    <h3>${ex.nome}</h3>
                    <p>${ex.reps}</p> <!-- Mostra só as reps aqui -->
                </div>
    ...
    const init = () => {
        ...
        dadosTreino.forEach((dia, index) => {
            ...
            elementos.bottomDaySelector.insertBefore(btn, elementos.botaoResetar); // Insere antes do reset
