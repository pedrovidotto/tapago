document.addEventListener('DOMContentLoaded', () => {
    ...
    const elementos = {
        themeToggleBtn: document.getElementById('theme-toggle-btn'), // Botão de tema
        inlineDaySelector: document.getElementById('day-selector-inline'), // Navegação inline
    ...
    let progresso = {};
    let diaAtivoIndex = 0;
    let currentTheme = localStorage.getItem('theme') || 'light'; // Padrão light
    ...
    const carregarProgresso = () => { progresso = JSON.parse(localStorage.getItem('minhaRotinaCleanV4')) || {}; };
    const salvarProgresso = () => { localStorage.setItem('minhaRotinaCleanV4', JSON.stringify(progresso)); };
    ...
    const init = () => {
        carregarProgresso();
        aplicarTema(currentTheme); // Aplica tema salvo ou padrão
